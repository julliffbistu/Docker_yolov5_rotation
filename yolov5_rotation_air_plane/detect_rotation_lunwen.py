...
def select_data(detection,class_list):
    select_list = []
    if detection[0] != []:
        for detect in detection:
            #print(detect)
            if detect[6] in class_list:
                select_list.append(detect)
    return select_list
...
def detect(weights='yolov5s.pt',  # model.pt path(s)
    ...
    for i,imgpath in enumerate(imglist):
        ...
        for i in range(len(weights)):
            print("******: ",weights[i])
            model = attempt_load(weights[i], map_location=device)  # load FP32 model
            ...
            if mulpicplus == "1":
                ...
                for i in range(mulpicplus):
                    ...
                    for j in range(mulpicplus):
                        ...
                        pred_temp = model(dicsrc,augment=augment)[0]
            ...
            if pred == []:
                continue
            print("name: ",str(weights[i]).split('/')[-1])

            if (str(weights[i]).split('/')[-1] == 'best_dang.pt'):
                class_list = [0.0, 8.0]#A,I,0.45,0.5
                prediction = select_data(pred, class_list)
                if prediction != []:
                    pred_son.append(prediction)

            if (str(weights[i]).split('/')[-1] == 'bestguang1_1_2.pt'):
                class_list = [9.0]#J,0.05
                prediction = select_data(pred, class_list)
                if prediction != []:
                    pred_son.append(prediction)

            if (str(weights[i]).split('/')[-1] == 'bestguang1_0_8.pt'):
                class_list = [6.0]#G,0.5
                prediction = select_data(pred, class_list)
                if prediction != []:
                    pred_son.append(prediction)
            
            if (str(weights[i]).split('/')[-1] == 'bestguang1_0_7.pt'):
                class_list = [3.0]#D,0.495
                prediction = select_data(pred, class_list)
                if prediction != []:
                    pred_son.append(prediction)

            if (str(weights[i]).split('/')[-1] == 'shateB.pt'):
                class_list = [1.0]#B,0.5
                prediction = select_data(pred, class_list)
                if prediction != []:
                    pred_son.append(prediction)

            if (str(weights[i]).split('/')[-1] == 'bestyi1_1_0.pt'):
                class_list = [4.0]#E,0.02
                prediction = select_data(pred, class_list)
                if prediction != []:
                    pred_son.append(prediction)

            if (str(weights[i]).split('/')[-1] == 'bestbing1_0_12.pt'):
                class_list = [10.0]#K,0.49
                prediction = select_data(pred, class_list)
                if prediction != []:
                    pred_son.append(prediction)

            if (str(weights[i]).split('/')[-1] == 'bestbing1_0_13.pt'):
                class_list = [7.0]#H,0.49
                prediction = select_data(pred, class_list)
                if prediction != []:
                    pred_son.append(prediction)
            
            if (str(weights[i]).split('/')[-1] == 'bestguang1_0_15.pt'):
                class_list = [2.0, 5.0]#C,F 0.49
                prediction = select_data(pred, class_list)
                if prediction != []:
                    pred_son.append(prediction)
        ...
        if pred_son == []:
            continue
        for data in pred_son:
            for elem in data:
                end_pred.append(elem)
