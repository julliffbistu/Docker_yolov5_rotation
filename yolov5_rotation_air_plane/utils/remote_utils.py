...
def rboxes2points(pred,CLASSES,score_thr=0):
    ...
    for label,bbox,score in zip(labels,bboxes,scores):
        ...
        #print("--------: ",[p1,p2,p3,p4])
        #print("ceter point is: ",int(p1[0]+(p3[0]-p1[0])/2),int(p1[1]+(p3[1]-p1[1])/2))
        #cv2.waitKey(100)
        object_dict['confidence']=float(score)
        results_list.append(object_dict)
    #results = new_list(results_list)
    return results_list
def draw_clsdet_rotation(img,cls_dets,vis_thresh=0.001):
    ...
    for i in range(len(cls_dets)):
        ...
        if score>vis_thresh:
            ...
            cv2.putText(img2,put_text,(int((x+w/2)+a),int((y+h/2)+a)),3,cv2.FONT_HERSHEY_PLAIN,(0,255,0),1)
            put_text1='label : {:}'.format(obj_label)
            cv2.putText(img2,put_text1,(int(x+w/2+ 40-a),int(y+h/2+40-a)),3,cv2.FONT_HERSHEY_PLAIN,(255,0,0),2)
    return img2
